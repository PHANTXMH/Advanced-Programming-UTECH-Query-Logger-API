...
public class LiveChatAvailabilityController {
    ...
    public ResponseEntity<ApiResponse> createAvailableDays(@Validated @RequestBody List<LiveChatAvailability> liveChatAvailabilities){
    ...
    public ResponseEntity<ApiResponse> deleteAvailableDayByID(@PathVariable("id") Long id){
        return new ResponseEntity<>(liveChatAvailabilityService.deleteAvailableDays(id), HttpStatus.OK);
    }

    @GetMapping(value = "/all/by/student/rep/id/{id}", produces = "application/json", consumes = "application/json")
    public ResponseEntity<List<LiveChatAvailability>> getAllLiveChatAvailableDaysByStudentRepID(@PathVariable("id") String id){
